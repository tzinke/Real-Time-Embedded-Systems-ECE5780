...
procedure Part5 is
    ...
    vTime, F1_Start, F1_Curr, F2_Curr, F3_Start, F3_Curr, F3_Dur, Before, After: Duration;
    G: Generator;
    RandOut : Float;
    ...
    task body Watchdog is
        ...
        Running, Printed : Boolean := False; -- Printed prevents multiple prints of warning
    begin
      loop
        select
            accept init do
              Start := Ada.Calendar.Seconds(Ada.Calendar.Clock); -- track when F3 began
              Running := True;
              Printed := False;
            end init;
        or
          ...
          if Running then
              Current := Ada.Calendar.Seconds(Ada.Calendar.Clock);
              if Current - Start >= 0.5 then -- F3 has been running longer than 0.5s
                  if Printed = False then
                    Put_Line("");
                    Put_Line("");
                    Put_Line("F3 has missed its deadline!");
                  end if;
                  Printed := True;
              end if;
          end if;
...
begin
    ...
    loop  -- Main loop
        loop  -- 1 second hyperperiod
            After := Ada.Calendar.Seconds(Ada.Calendar.Clock);
            exit when After - Before >= 1.000000000;
        end loop;
        ...
        F3_Dur := 0.2;
        RandOut := Random(G);
        if RandOut <= 0.10 then --Causes occasional missed deadline
            F3_Dur := F3_Dur + Duration(0.3 + RandOut);
        end if;

        loop
          F3_Curr := Ada.Calendar.Seconds(Ada.Calendar.Clock);
          exit when F3_Curr - F3_Start >= F3_Dur; -- Run F3 for 0.2s OR 0.2 + random float
        end loop;
        F3(Currtime => vTime + (F3_Start - F1_Start), StartF1 => F3_Start, FinishF3 => F3_Curr);
        Watchdog.stop;

        Before := Before + 1.0; -- Keep fixed reference point to prevent drift

        -- If more than 1s has passed since start of hyperperiod, start at next second
        if Ada.Calendar.Seconds(Ada.Calendar.Clock) - After > 1.0 then
          Before := Before + 1.0;
          vTime := vTime + 1.0;
          Put_Line("");
          Put_Line("");
          Put_Line("Re-syncing schedule");
        end if;
    end loop; --Main loop
end Part5;
