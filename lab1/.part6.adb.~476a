    ...
    begin
        ...
        loop
            select
                ...
                -- This uses relative durations rather than a delay
                -- so Producer can close quicker
                if Start = 0.0 then
                    Rand25 := Random(G);

                    if Rand25 < 3 then -- This acceptance condition will cause shorter periods
                                       -- than that of the consumer.
                        Wait := Duration(Rand25);
                        Start := Ada.Calendar.Seconds(Ada.Calendar.Clock);
                    end if;
                else
                    if Ada.Calendar.Seconds(Ada.Calendar.Clock) >= Start + Wait then
        ...
        loop
            ...
            if Rand25 < 5 then
                ...
                if sum >= 100 then
                    ...
                    Producer.stop;
                    -- Following loop prevents FIFO from closing while producer is still
                    -- pending a Put operation
                    loop
                        null;
                    ...
                    FIFO.stop;
                    -- This loop just for consistency
                    loop
                        null;
                        exit when FIFO'Terminated;
                    end loop;
